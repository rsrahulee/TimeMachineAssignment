...
-(void)calculateDays
{
    ...
    classWeek* compareWeek,*compareWeek1;
    startDay=@"Monday";
    
    for(i=0;i<=size-1;i++){
        compareWeek = [weekChart objectAtIndex:i];
        
        if (i==6) {
            compareWeek1 = [weekChart objectAtIndex:0];
        }
        else{
            compareWeek1 = [weekChart objectAtIndex:i+1];
        }
        
        
        if((compareWeek.startTime==compareWeek1.startTime)&&
           ([compareWeek.startState isEqualToString: compareWeek1.startState]) &&
           (compareWeek.endTime==compareWeek1.endTime)&&
           ([compareWeek.endState isEqualToString:compareWeek1.endState])){
        ...
        {
            if ([startDay isEqualToString:compareWeek.Day]){
                
                Day=startDay;
            }
            else
            {
                Day=[(startDay)stringByAppendingString:(compareWeek.Day)];
                
            }
            
            flag=0;
            
            timeDetails =[NSString stringWithFormat:@" %d %d",compareWeek.startTime , compareWeek.endTime];
            NSString *DisplayOutput=[(Day) stringByAppendingString:timeDetails];
            NSLog(@"display %@",DisplayOutput);
            startDay=compareWeek1.Day;
...
{
    if(textField==txtstart1)
    {
        [txtstart2 resignFirstResponder];
        
    }
    else if(textField==txtstart2)
    {
        [txtstart2 resignFirstResponder];
